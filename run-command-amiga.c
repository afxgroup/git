...
int start_command(struct child_process *cmd)
{
	...
	if (cmd->no_stdin) {
		...
		if (fhin < 0) {
			failed_errno = errno;
			error_errno("cannot dup stdin pipe for spawn");
			goto fail_spawn;
		}
	}
	else if (cmd->in) {
		...
		if (fhin < 0) {
			failed_errno = errno;
			error_errno("cannot dup stdin for spawn");
			goto fail_spawn;
		}
	}
	...
	if (fhin < 0) {
		failed_errno = errno;
		error_errno("cannot open/dup stdin for spawn");
		goto fail_spawn;
	}
	...
	if (fherr < 0) {
		failed_errno = errno;
		error_errno("cannot dup stderr for spawn");
		goto fail_spawn;
	}
	...
	if (fhout < 0) {
		failed_errno = errno;
		error_errno("cannot dup stdout for spawn");
		goto fail_spawn;
	}

	if (cmd->git_cmd) {
		/*
		 * AmigaOS4/clib4: invoking `git remote-*` via the `git` wrapper can
		 * lose stdio pipe wiring (child sees TTY on stdout). Execute remote
		 * helpers directly to avoid nested spawn/wait deadlocks.
		 */
		if (sargv[0] && !strncmp(sargv[0], "remote-", 7)) {
			strvec_pushf(&nargv, "git-%s", sargv[0]);
			strvec_pushv(&nargv, sargv + 1);
			cmd->args.v = nargv.v;
			use_helper_fd_env = 1;
			trace_printf("[start_command] using direct remote helper path\n");
		} else {
	...
	trace_printf("[start_command] fd diagnostic: isatty(fhin=%d)=%d isatty(fhout=%d)=%d isatty(fherr=%d)=%d parent_tty(0=%d,1=%d,2=%d)\n",
		     fhin, isatty(fhin), fhout, isatty(fhout), fherr, isatty(fherr),
	...
	/*
	 * Determine env right before spawn, after all potential strvec_pushf() on
	 * cmd->env to avoid stale pointers when the vector reallocates.
	 */
	 ...
	 * deltaenv as a COMPLETE environment replacement when non-NULL, so we
	 * cannot add GIT_AMIGA_CLOSE_FDS via cmd->env without losing GIT_DIR
	 * and other repository variables from the parent environment.  Instead,
	 * inject the variable directly into the parent's own env for the
	 * duration of the spawnvpe call; the child inherits the full env plus
	 * our addition, and we clean up immediately after.
	 */
	if (need_in && !use_helper_fd_env) {
		char cfd_buf[32];
		snprintf(cfd_buf, sizeof(cfd_buf), "%d", fdin[1]);
		setenv("GIT_AMIGA_CLOSE_FDS", cfd_buf, 1);
		env_to_use = environ; /* refresh after possible realloc by setenv */
	}

	cmd->pid = spawnvpe(cmd->args.v[0], cmd->args.v,
				  env_to_use,
				  dir_to_use, spawn_fhin, spawn_fhout, spawn_fherr);
	failed_errno = errno;
...
after_amiga_fail:
	...
	trace_printf("[start_command] post-spawn parent fd diagnostic: isatty(0)=%d isatty(1)=%d isatty(2)=%d\n",
		     isatty(0), isatty(1), isatty(2));
...
int finish_async(struct async *async)
{
	...
	if (join_result)
		error("pthread_join failed with error %d", join_result);
...
int prepare_auto_maintenance(struct repository *r, int quiet, struct child_process *maint)
{
