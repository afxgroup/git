	...
	subprocess_start_fn startfn)
{
	...
	err = startfn(entry);
	if (err) {
		_error("initialization for subprocess '%s' failed", cmd);
			     ...
			     int *chosen_version)
{
	...
	if (packet_write_fmt_gently(process->in, "%s-client\n",
				    welcome_prefix))
		return _error("Could not write client identification");
	for (i = 0; versions[i]; i++) {
		if (packet_write_fmt_gently(process->in, "version=%d\n",
					    versions[i]))
			return _error("Could not write requested version");
	}
	if (packet_flush_gently(process->in))
		return _error("Could not write flush packet");

	if (!(line = packet_read_line(process->out, NULL)) ||
	    !skip_prefix(line, welcome_prefix, &p) ||
	    strcmp(p, "-server"))
		return _error("Unexpected line '%s', expected %s-server",
			     line ? line : "<flush packet>", welcome_prefix);
	if (!(line = packet_read_line(process->out, NULL)) ||
	    !skip_prefix(line, "version=", &p) ||
	    strtol_i(p, 10, chosen_version))
		return _error("Unexpected line '%s', expected version",
			     line ? line : "<flush packet>");
	if ((line = packet_read_line(process->out, NULL)))
		return _error("Unexpected line '%s', expected flush", line);
	...
	if (!versions[i])
		return _error("Version %d not supported", *chosen_version);
				  ...
				  unsigned int *supported_capabilities)
{
	...
	for (i = 0; capabilities[i].name; i++) {
		if (packet_write_fmt_gently(process->in, "capability=%s\n",
					    capabilities[i].name))
			return _error("Could not write requested capability");
	}
	if (packet_flush_gently(process->in))
		return _error("Could not write flush packet");
